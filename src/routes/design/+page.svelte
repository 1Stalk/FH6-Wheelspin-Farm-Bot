<script lang="ts">
  /**
   * /design — Design Sandbox & Main Interface logic.
  ...
  async function connect() {
    ...
    try {
      ...
      console.warn("Could not retrieve auth token, using default:", e);
    }
    ...
    ws.onclose = () => {
      connected = false;
      ws = null;
      addLog("warn", "Disconnected from bot server. Reconnecting...");
  ...
  function onRunNav(e: CustomEvent<{ target: string }>) {
    send({ cmd: "run_nav", target: e.detail.target });
  }

  function onTestGamepad() {
    send({ cmd: "test_gamepad" });
  }
...
<div class="design-shell">
  ...
  <div class="app-shell">
    ...
    <div class="app-footer">
      ...
      <span class="footer-right mono">v1.2.0 · FH6WheelspinFarmBot</span>
    </div>
  </div>
