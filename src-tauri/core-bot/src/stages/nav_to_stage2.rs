...
fn find_and_select_subaru_car(ctx: &BotFSMContext, scan_only: bool) -> bool {
    ...
    for _scroll in 0..max_scrolls {
        ...
        if let Some((cursor_col, cursor_row)) = cursor_pos {
            ...
            if is_car && !is_class_b {
                ctx.logger.info("[Nav→S2] Cursor is already pointing to the target Subaru! Selecting it.");
                drop(capture);
                {
                    let mut pad = ctx.pad.lock().unwrap();
                    pad.press_a();
        ...
        for r in 0..3 {
            for c in 0..4 {
                ...
                let matches = find_all_matches(ctx, &frame, "subaru_impreza_1998.png", 0.85, Some((cell_rx, cell_ry, cell_rw, cell_rh)));
                if !matches.is_empty() {
